...
ui = fluidPage(
  fluidRow(
    column(3,
	    ...
	    checkboxInput("heat", "Licitáraránynál heatmap", value = F, width = NULL),
...
server = function(input, output, session){
  ...
  if(input$var == "kat"){
    ...
    leaflet(dfs) %>%
      addTiles() %>%
    	addHeatmap(lng=dfs[dfs$kat =="alatt",]$lon,lat=dfs[dfs$kat =="alatt",]$lat,group="Alatt",intensity = 1,max=100,radius = 15,blur = 10) %>%
    	addHeatmap(lng=dfs[dfs$kat =="felett",]$lon,lat=dfs[dfs$kat =="felett",]$lat,group="Felett",intensity = 1,max=100,radius = 15,blur = 10) %>%
    	addHeatmap(lng=dfs[dfs$kat =="megegyezik",]$lon,lat=dfs[dfs$kat =="megegyezik",]$lat,group="Megegyezik",intensity = 1,max=100,radius = 15,blur = 10) %>%
    	addHeatmap(lng=dfs[is.na(dfs$kat),]$lon,lat=dfs[is.na(dfs$kat),]$lat,group="Nincs licit",intensity = 1,max=100,radius = 15,blur = 10) %>%
  		addLayersControl(
  			#baseGroups = c("OSM (default)", "Toner", "Toner Lite"),
